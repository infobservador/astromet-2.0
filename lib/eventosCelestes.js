...
function calidadVisibilidad(latObservador, declinacionRadiante) {
  ...
  if (alturaMaxima >= 50) return { nivel: "alta", alturaMaxima: Math.round(alturaMaxima) };
  if (alturaMaxima >= 20) return { nivel: "media", alturaMaxima: Math.round(alturaMaxima) };
  if (alturaMaxima >= 0) return { nivel: "baja", alturaMaxima: Math.round(alturaMaxima) };
  return { nivel: "no visible", alturaMaxima: Math.round(alturaMaxima) };
}
