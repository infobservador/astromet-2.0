...
export async function generarPdfReporte({ data, advice, descripcionNoche, lugarNombre, coords, fecha, desdeHora, hastaHora, banderaRoja, motivosBanderaRoja }) {
  ...
  // Bandera roja de seguridad (si corresponde, va primero y bien destacada)
  if (banderaRoja) {
  ...
  parrafo(`Fecha: ${fecha}   Horario: ${desdeHora}:00 a ${hastaHora}:00 hs`);
  linea(2);
  ...
  if (hayEventos) {
    ...
    for (const l of ec.lluviasMeteoros || []) {
      parrafo(`Lluvia de meteoros ${l.nombre}${l.esPico ? " (hoy es el pico)" : " (activa)"} — visibilidad desde el sur: ${l.visibilidadSur}`);
    }
