...
function armarPrompt({ lugarNombre, fecha, desdeHora, hastaHora, bloques, solLuna, bortleInfo, eventosCelestes }) {
  ...
  for (const l of eventosCelestes?.lluviasMeteoros || []) {
    lineasEventos.push(
      `- Lluvia de meteoros "${l.nombre}"${l.esPico ? " (HOY ES EL PICO)" : " (activa, no es el pico exacto)"}, calidad de visibilidad desde esta latitud: ${l.calidadVisibilidad} (el radiante alcanza hasta ${l.alturaMaximaGrados}° de altura en el cielo). IMPORTANTE: las lluvias de meteoros solo se ven bien a simple vista bajo cielos REALMENTE oscuros (Bortle bajo); con contaminación lumínica moderada o alta la mayoría de los meteoros no se van a poder ver, por más que la altura del radiante sea buena.`
    );
  }
...
- Si hay eventos celestes reales listados arriba (lluvia de meteoros, conjunción, eclipse), podés mencionarlos como un dato más de interés — pero NO los conviertas en el centro ni el título de toda la actividad. Una lluvia de meteoros, en particular, es solo un ingrediente más de la noche (y depende de un cielo muy oscuro para verse, como ya se aclaró), no el motivo principal de la salida. Si la lista dice que no hay ningún evento, NO menciones ninguno, ni inventes uno.
- VENTANA HORARIA REAL DE LA ACTIVIDAD: aunque la franja horaria elegida pueda extenderse hasta la madrugada, la actividad turística de astroturismo ocurre en la práctica mayormente entre la puesta del sol y la 1 de la madrugada aproximadamente — después de esa hora ya casi no hay turistas despiertos observando. Cuando recomiendes qué observar, priorizá lo que esté realmente visible y bien ubicado en el cielo EN ESA FRANJA TEMPRANA (atardecer-1am), no generalices diciendo "toda la noche" ni recomiendes objetos que recién se ven bien mucho más tarde (por ejemplo, constelaciones que en esta época del año recién salen después de la 1am no deberían presentarse como parte central de la actividad).
...
2. Dé UNA recomendación operativa concreta y accionable, adaptada específicamente a estos datos: qué observar (siempre objetos brillantes/accesibles, nunca objetos débiles, y priorizando cualquier evento celeste real de la lista de arriba si lo hay), cómo estructurar la actividad, y el componente de mitología/cultura correspondiente. Si el cielo no acompaña, sumá UNA actividad alternativa concreta, elegida entre estas (no las listes todas juntas, elegí la que mejor encaje): proyección de documentales, juegos interactivos de orientación estelar, realidad aumentada, realidad virtual, cata de productos regionales con historia cultural, charla de mitología y cosmovisiones de distintas culturas, o material táctil/auditivo del cielo para personas ciegas o con baja visión. La mitología no se reemplaza por estas alternativas, se refuerza.
